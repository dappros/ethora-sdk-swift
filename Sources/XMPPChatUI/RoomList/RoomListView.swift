...
public class RoomListViewModel: ObservableObject {
    ...
    public init(
        ...
        appId: String? = nil,
        apiBaseURL: URL = URL(string: "https://api.messenger-dev2.vitall.com/v1")!,
        conferenceDomain: String = "conference.xmpp.messenger-dev2.vitall.com"
    ) {
        self.client = client
        self.currentUserId = currentUserId
...
extension RoomListViewModel {
    ...
    public convenience init(client: XMPPClient, currentUserId: String, config: ChatConfig) {
        ...
        let conference = config.xmppSettings?.conference
            ...
            ?? "conference.xmpp.messenger-dev2.vitall.com"
        self.init(
            client: client,
            currentUserId: currentUserId,
            appId: resolvedAppId,
